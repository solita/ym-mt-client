...
import ResultView from './WasteBatchSummaryResultView';
import { getJsonData } from '../../services/ApiService';
import { FETCH_FACILITIES, WASTE_BATCH } from '../../services/endpoints';
...
class WasteBatchSummary extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...initialSearchState,
  ...
  onSubmit = () => {
    ...
    getJsonData(WASTE_BATCH, config)
      .then(res => {
        this.setState({
          loading: false,
          results: res
    ...
    facilityIds.map(facilityId => {
      ...
      return (
        <button
          ...
          type="button"
          className={'searchTerms__Button'}
          onClick={() => this.deleteSearchTerm('facilityIds')(facility.businessId)}
        >
          {facility.name}
          <span className={'searchTerms__Button__x'} aria-label={this.props.t('Poista hakuehto')}>
            &times;
  ...
  render() {
    ...
    const { classifications, service, facilityIds, facilityList, year, searchDone } = this.state;
                    ...
                    <Col span={6} sm={6} xs={12}>
                      <Loader loading={this.state.loadingFacilities}>
                        <label>
                          ...
                          <select onChange={this.toggleSearchTerm} name="facilityIds" value="">
                            <option value="" disabled hidden>
                        ...
                        <div className={styles.searchTerms__Container}>
                          {this.facilitySearchTermsView(facilityIds)}
                        </div>
                      </Loader>
                    </Col>
                    <Col span={6} sm={6} xs={12}>
                      <label>
                        <span className={styles.defaultLabelSpan}>
                          <strong>{t('Raportointivuosi')}</strong>
                        </span>
